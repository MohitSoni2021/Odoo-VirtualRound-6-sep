...
// New: SMTP-based sender (no OAuth)
const sendVerificationEmailSMTP = async (userEmail, otp, username) => {
  ...
  if (!smtpHost || !smtpPort || !smtpUser || !smtpPass) {
    throw new Error("Missing SMTP configuration in .env file (SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS)");
  }
  ...
  try {
    const transporter = nodemailer.createTransport({
      host: smtpHost,
      port: smtpPort,
      secure: smtpPort === 465, // true for 465, false for 587/25/2525
      auth: {
        user: smtpUser,
        pass: smtpPass,
      },
    ...
    throw new Error(`Failed to send SMTP email: ${error.message}`);
  }
};

export { sendVerificationEmail, sendVerificationEmailSMTP };